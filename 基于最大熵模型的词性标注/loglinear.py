...
class LogLinearModel(object):
    ...
    def SGD(self, trainset, devset, file, epochs, batch_size,
        ...
        traindata = [(wiseq, i, ti) for wiseq, tiseq in trainset
                     for i, ti in enumerate(tiseq)]
        #训练集大小
        n = len(traindata)
        ...
        for epoch in range(1, epochs + 1):
            ...
            #分批
            batches = [traindata[i:i+batch_size] for i in range(0,n,batch_size)]
            nb = len(batches)
        ...
        print("mean time:" + str(total_time / epoch))

    def update(self, batch, lmbda, n, eta):
        gradients = defaultdict(float)

        for wiseq, i, ti in batch:
            fv = self.instantialize(wiseq, i, ti)
            ...
            #获取每个词性对应的所有特征
            fvs = [self.instantialize(wiseq, i, ti) for ti in range(self.nt)]
            scores = np.array([self.f(fv) for fv in fvs])
